  ...
  <q-layout view="hHh LpR fff">
    ...
    <q-drawer show-if-above v-model="rightDrawerOpen" side="right" elevated>
      ...
      <q-card-section class="q-pa-md">
          <q-card class="card" bordered v-for="exam in usersExamsRef" :key="exam.id">
            <q-card-section>
              <q-item-label>Venue: <b>{{exam.venue}}</b></q-item-label>
              <q-item-label>Date: <b>{{ formatDate(exam.startTime)}} </b></q-item-label>
              <q-item-label>Time: <b>{{ formatTime(exam.startTime) + ' - ' + formatTime(exam.endTime) }} </b></q-item-label>
              <q-item-label>Type: <b>{{exam.type}}</b></q-item-label>
              <q-item-label>Note: <b>{{exam.note}}</b></q-item-label>
            </q-card-section>
          </q-card>
      </q-card-section>
